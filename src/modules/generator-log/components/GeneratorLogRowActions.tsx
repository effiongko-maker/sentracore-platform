...
interface GeneratorLogRowActionsProps {
  entry: GeneratorLog;
  onView: (entry: GeneratorLog) => void;
  onEdit: (entry: GeneratorLog) => void;
  /** When false, hide Edit (view-only actors). */
  canMutate?: boolean;
}

export function GeneratorLogRowActions({
  entry,
  onView,
  onEdit,
  canMutate = true,
}: GeneratorLogRowActionsProps) {
  ...
  const label = entry.generator || entry.id;
