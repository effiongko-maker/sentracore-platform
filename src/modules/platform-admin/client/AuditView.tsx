    ...
    <>
      ...
      {error && !events ? (
        ...
        <>
          ...
          {moreError ? <p className="ac-form-error" role="alert">{moreError}</p> : null}
          {next ? (
            <div style={{ marginTop: 16 }}>
              <button type="button" className="ac-btn ac-btn-secondary" onClick={more} disabled={loadingMore}>
                {loadingMore ? "Loading…" : "Load older changes"}
              </button>
            </div>
          ) : (
            <p className="ac-secondary" style={{ marginTop: 16 }}>End of history.</p>
          )}
        </>
      )}
    </>
