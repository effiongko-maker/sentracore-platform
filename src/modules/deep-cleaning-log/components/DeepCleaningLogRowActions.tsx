...
interface DeepCleaningLogRowActionsProps {
  entry: DeepCleaningLog;
  onView: (entry: DeepCleaningLog) => void;
  onEdit: (entry: DeepCleaningLog) => void;
  canMutate?: boolean;
}

export function DeepCleaningLogRowActions({
  entry,
  onView,
  onEdit,
  canMutate = true,
}: DeepCleaningLogRowActionsProps) {
  ...
  const label = entry.area || entry.id;
