...
export function PlatformFinanceReceivablesPage() {
  ...
  return <div className="pf-page">
    ...
    {!busy && !error ? <div className="pf-table-wrap"><table className="pf-table"><thead><tr><th>Invoice</th><th>Customer</th><th>Invoice Date</th><th>Due Date</th><th>Original Amount</th><th>Outstanding</th><th>Status</th></tr></thead><tbody>
      {filtered.length === 0 ? <tr><td colSpan={7} className="pf-empty">No receivables.</td></tr> : filtered.map((row) => <tr key={row.id} onClick={() => setSelected(row)} style={{ cursor: "pointer" }}><td>{row.invoiceReference}</td><td>{row.counterpartyDisplayName}</td><td>{date(row.invoiceDate)}</td><td>{date(row.dueDate)}</td><td>{money(row.originalAmount, row.currency)}</td><td>{money(row.outstandingAmount, row.currency)}</td><td><span className={`pf-pill ${row.status === "overdue" ? "is-danger" : "is-success"}`}>{row.status === "overdue" ? "Overdue" : "Open"}</span></td></tr>)}
    </tbody></table></div> : null}
    {selected ? <div className="pf-drawer-backdrop" role="presentation" onMouseDown={() => setSelected(null)}><aside className="pf-drawer" role="dialog" aria-modal="true" aria-label="Receivable detail" onMouseDown={(e) => e.stopPropagation()}><header className="pf-drawer-header"><div><p className="pf-ov-eyebrow">Receivable</p><h2>{selected.invoiceReference}</h2></div><button className="pf-btn is-ghost" onClick={() => setSelected(null)} aria-label="Close"><X size={18}/></button></header><div className="pf-drawer-body"><section className="pf-rev-card"><h3>{selected.counterpartyDisplayName}</h3><p>{selected.companyName}</p><p>Original: {money(selected.originalAmount, selected.currency)}</p><p>Outstanding: {money(selected.outstandingAmount, selected.currency)}</p><p>Due: {date(selected.dueDate)}</p></section><section className="pf-rev-card"><h3>Source &amp; accounting</h3><p><Link href={`/platform-finance/invoices/${selected.invoiceId}`}>Invoice {selected.invoiceReference}</Link></p><p>→ Recognition transaction</p><p>→ <Link href={`/platform-finance/accounting/journal/${selected.journalEntryId}`}>Posted journal</Link></p></section><p className="pf-state-message">Settlement and receipt allocation are not part of this phase.</p></div></aside></div> : null}
  </div>;
}
