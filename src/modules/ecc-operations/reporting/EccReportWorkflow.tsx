...
import { ECC_REPORT_WIZARD_STEPS } from "./constants";
import type { EccReportWizardStep } from "./types";

export function EccReportWorkflow({
  step,
  onStepClick,
}: {
  ...
  const currentIndex = ECC_REPORT_WIZARD_STEPS.findIndex((s) => s.id === step);

  return (
    <ol className="rp-workflow" aria-label="Report generation steps">
      {ECC_REPORT_WIZARD_STEPS.map((item, index) => {
