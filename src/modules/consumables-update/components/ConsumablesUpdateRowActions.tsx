...
interface ConsumablesUpdateRowActionsProps {
  entry: ConsumablesUpdate;
  onView: (entry: ConsumablesUpdate) => void;
  onEdit: (entry: ConsumablesUpdate) => void;
  canMutate?: boolean;
}

export function ConsumablesUpdateRowActions({
  entry,
  onView,
  onEdit,
  canMutate = true,
}: ConsumablesUpdateRowActionsProps) {
  ...
  const label = entry.itemName || entry.id;
