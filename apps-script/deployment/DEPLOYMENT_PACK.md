 ...
 * EnergyReadingRepository.gs
 *
 * Sheet: EnergyReadings
 * Columns (row 1 headers — exact order):
 *   id | date | meter | reading | remarks |
 *   createdAt | updatedAt | createdByUserId | updatedByUserId
 *
 * Mirrors GeneratorLogRepository pattern. Soft rows only — never delete.
 */

var EnergyReadingRepository = (function () {
  var SHEET_NAME = "EnergyReadings";
  var HEADERS = [
    "id",
    "date",
    "meter",
  ...
  /** Prefer YYYY-MM-DD for calendar date cells. */
  function cellDate_(value) {
    if (value == null || value === "") return "";
    if (Object.prototype.toString.call(value) === "[object Date]") {
      if (!isFinite(value.getTime())) return "";
  ...
  function nextId_() {
    ...
    var max = 0;
    for (var i = 0; i < all.length; i++) {
      var match = String(all[i].id || "").match(/ENRG-(\d+)/i);
      if (match) {
        var n = parseInt(match[1], 10);
        if (n > max) max = n;
      }
  ...
  function create(payload) {
    ...
    var row = [
      ...
      payload.date || "",
      payload.meter || "",
      payload.reading != null ? payload.reading : 0,
      payload.remarks || "",
      now,
      now,
      payload.createdByUserId || "",
    ...
    return (
      getById(id) || {
        ...
        date: payload.date || "",
        meter: payload.meter || "",
        reading: payload.reading != null ? payload.reading : 0,
        remarks: payload.remarks || "",
        createdAt: now,
        updatedAt: now,
        createdByUserId: payload.createdByUserId || "",
  ...
  function update(id, payload) {
    ...
    var updated = {
      id: id,
      date: payload.date != null ? payload.date : current.date,
      meter: payload.meter != null ? payload.meter : current.meter,
      reading: payload.reading != null ? payload.reading : current.reading,
      remarks: payload.remarks != null ? payload.remarks : current.remarks,
      createdAt: current.createdAt,
      updatedAt: new Date().toISOString(),
      createdByUserId: current.createdByUserId || "",
 ...
 * FumigationLogRepository.gs
 *
 * Sheet: FumigationLogs
 * Columns (row 1 headers — exact order):
 *   id | date | facilityId | areaTreated | pestType | vendor | nextDueDate |
...
var FumigationLogRepository = (function () {
  var SHEET_NAME = "FumigationLogs";
  var HEADERS = [
    "id",
    "date",
    "facilityId",
  ...
  function nextId_() {
    ...
    var max = 0;
    for (var i = 0; i < all.length; i++) {
      var match = String(all[i].id || "").match(/FLOG-(\d+)/i);
      if (match) {
        var n = parseInt(match[1], 10);
        if (n > max) max = n;
      }
  ...
  function create(payload) {
    ...
    return (
      getById(id) || {
        ...
        date: payload.date || "",
        facilityId: payload.facilityId || "",
        areaTreated: payload.areaTreated || "",
    ...
    var updated = {
      ...
      areaTreated:
        payload.areaTreated != null
          ? payload.areaTreated
          : current.areaTreated,
      pestType: payload.pestType != null ? payload.pestType : current.pestType,
      vendor: payload.vendor != null ? payload.vendor : current.vendor,
      nextDueDate:
 ...
 * Sheet: GeneratorLogs
 * Columns (row 1 headers — exact order):
 *   id | date | generator | startedAt | endedAt | hours | fuelUsed |
 *   remarks | createdAt | updatedAt | createdByUserId | updatedByUserId
 *
 * Mirrors FacilityRepository pattern. Soft rows only — never delete.
 */

var GeneratorLogRepository = (function () {
  var BUILD_MARKER = "2026-09-10-generator-log-v1";
  var SHEET_NAME = "GeneratorLogs";
  var HEADERS = [
    "id",
    "date",
    "generator",
  ...
  function nextId_() {
    ...
    var max = 0;
    for (var i = 0; i < all.length; i++) {
      var match = String(all[i].id || "").match(/GENLOG-(\d+)/i);
      if (match) {
        var n = parseInt(match[1], 10);
        if (n > max) max = n;
      }
  ...
  function create(payload) {
    ...
    var row = [
      ...
      payload.startedAt || "",
      payload.endedAt || "",
      payload.hours != null ? payload.hours : 0,
      payload.fuelUsed != null ? payload.fuelUsed : 0,
      payload.remarks || "",
      now,
      now,
      payload.createdByUserId || "",
    ...
    return (
      getById(id) || {
        ...
        hours: payload.hours != null ? payload.hours : 0,
        fuelUsed: payload.fuelUsed != null ? payload.fuelUsed : 0,
        remarks: payload.remarks || "",
        createdAt: now,
        updatedAt: now,
        createdByUserId: payload.createdByUserId || "",
  ...
  function update(id, payload) {
    ...
    var updated = {
      ...
      date: payload.date != null ? payload.date : current.date,
      generator:
        payload.generator != null ? payload.generator : current.generator,
      startedAt:
        payload.startedAt != null ? payload.startedAt : current.startedAt,
      endedAt: payload.endedAt != null ? payload.endedAt : current.endedAt,
      hours: payload.hours != null ? payload.hours : current.hours,
      fuelUsed: payload.fuelUsed != null ? payload.fuelUsed : current.fuelUsed,
      remarks: payload.remarks != null ? payload.remarks : current.remarks,
      createdAt: current.createdAt,
      updatedAt: new Date().toISOString(),
      createdByUserId: current.createdByUserId || "",
