/**
 * Phase 29 browser — deep-link paths (no list search dependency).
 */
...
async function openDetail(page, workId) {
...
async function main() {
  const fixtures = JSON.parse(
    readFileSync("/tmp/phase29-fixtures.json", "utf8")
  );
  const env = loadEnv();
  ...
  try {
    ...
    await openDetail(page, fixtures.requiresWo);
    ...
    if (out.scenarios.detailCreateVisible.visible) {
      ...
      out.scenarios.workDetailCreate = { result: "FAIL", reason: "button hidden" };
    }

    await openDetail(page, fixtures.noRequiresWo);
    out.scenarios.detailNoRequiresWo = {
      createVisible:
        (await page.getByRole("button", { name: /Create Work Order/i }).count()) >
        0,
