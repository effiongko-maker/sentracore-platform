...
assert((summaryClient.match(/postToAppsScriptData\(/g) ?? []).length === 2, "summary client exposes exactly two Apps Script calls");
...
assert(summaryClient.includes("Assignment Summary operational identity mismatch"), "Assignment identity echo mismatch fails closed");
